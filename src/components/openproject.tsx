...
const loadExamples = <P extends PLang>(
    pLang: P,
    metadata: ReadonlyArray<ProjectMetadata>
): ProjectStore[P] => {
    return {
        files: metadata.reduce(
            (result, m) => ({
                ...result,
                [m.file]: {
                ...
                <div className="openproject-inner">
                    {projectStore[props.curPLang].order.map((file) => {
                        ...
                        return (
                            ...
                            >
                                <FontAwesomeIcon
                                    className="openproject-project-icon"
                                    icon={faFileCode}
                                />
                                <div>
                                    ...
                                    <a
                                        className="openproject-project-subtitle"
                                        href={example.metadata.author.url}
                                        target="_blank"
                                        onClick={(event) => {
