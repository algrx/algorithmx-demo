...
/// <reference path="../declarations/split.d.ts"/>
...
const App: React.FC<StateProps & DispatchProps> = props => {
  return <>
    <div className='container'>
      <Split className='split-container-x'
        ...
        <div className='split' style={{ position: 'relative' }}>
          <Split className='split-container-y'
            direction='vertical'
            sizes={[70, 30]}
            gutterSize={2}
            onDrag={() => props.dispatchResize()}>
    ...
    {props.popupState.isOpen
      ? props.popupState.popup === Popup.OpenProject ? <OpenProjectConnected/>
        : <></>
      : <></>}
  </>
}
