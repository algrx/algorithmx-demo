...
import { projectStore, defaultProjectFiles, ProjectActionType, ActionOpenProject } from './openproject'
...
const initAce = (el: HTMLElement, state: EditorState, curPLang: PLang): AceState => {
  ...
  const createSession = (pLang: PLang): Ace.IEditSession => {
    ...
    const session = new Ace.EditSession(hasSavedCode ? state.savedCode![pLang]!
  ...
  return (
    <div className='editor-container'>
      ...
      <div className='editor' ref={el => {
        if (el && !props.editorState.aceState) props.dispatchInit(initAce(el, props.editorState, props.curPLang))
        return el
      }}>
      </div>
    </div>
...
export const editorReducer: Reducer<EditorState> = (state = initEditorState, action: AnyAction): EditorState => {
