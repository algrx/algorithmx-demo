...
export const mapExecuteDispatchToProps = (dispatch: Dispatch): ExecuteDispatchProps => ({
  dispatchAlgrx: algrxState => dispatch<ActionAlgrx>({ type: ExecuteActionType.Algrx, algrxState }),
  dispatchExecute: executeId => dispatch<ActionExecute>({ type: ExecuteActionType.Execute, executeId }),
...
export const executeReducer: Reducer<ExecuteState> = (
  ...
  if (action.type === ExecuteActionType.Log) {
    return {...state, log: state.log.concat(processLog([{
  ...
  if (state.algrxState) {
    if (action.type === AppActionType.ChangePLang) {
      return {...reset(state),
        log: getPLangLog((action as ActionChangePLang).newPLang)
      }
    }
